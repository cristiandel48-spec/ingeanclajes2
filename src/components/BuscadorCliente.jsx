...
export default function BuscadorCliente({ label = "Empresa", valor, clientes, onEscribir, onElegir, ayuda }) {
  ...
  return (
    <div ref={caja} style={{ position: "relative" }}>
          ...
          {encontrados.map((c, i) => (
            <button
              ...
              style={{
                ...
                background: i === resaltado ? "#f1f5f9" : "#fff",
                borderBottom: i < encontrados.length - 1 ? "1px solid #f1f5f9" : "none",
                fontFamily: "inherit",
              }}
            >
              <div style={{ fontSize: 12.5, fontWeight: 600, color: "#1a1a2e" }}>{c.nombre}</div>
      ...
      {ayuda && <div style={{ fontSize: 10.5, color: "#94a3b8", marginTop: 3 }}>{ayuda}</div>}
    </div>
  );
}
