...
export default function GoogleMeasureWorkspace({ queryValue, onQueryChange, measurements, onChange, mapView, onMapViewChange }){
  ...
  const redraw = ()=>{
    ...
    (measurements||[]).forEach(seg=>{
      ...
      const label = createMapLabelOverlay(gm, mapRef.current, startPos, endPos, labelHtml, seg.tipo === "CON" ? "#a16207" : seg.tipo === "LVV" ? "#15803d" : seg.tipo === "ESC" ? "#574e44" : "#574e44");
      const sync = ()=>{
  ...
  return (
    <div style={{background:"#fff",border:"1px solid #e8dfd2",borderRadius:12,padding:20}}>
      <div style={{display:"grid",gridTemplateColumns:"1.3fr 1fr",gap:16,alignItems:"start",marginBottom:12}}>
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:8,marginBottom:10}}>
            ...
            <button onClick={centerMap} style={{...B("#2b2622"),fontSize:12}}>Ver aquí</button>
            <button onClick={startMeasurement} style={{...B(measureMode?"#2b2622":"#e8dfd2", measureMode?"#ffffff":"#60b4ff"),fontSize:12}}>{measureMode?"Esperando clics...":"Activar medición"}</button>
          </div>
          <div style={{fontSize:11,color:"#756a5e",lineHeight:1.6}}>Haz clic en dos puntos del mapa y el sistema calcula la distancia real automáticamente en metros. Luego solo nombras el tramo y su tipo.</div>
        </div>
        <div style={{background:"#fbf8f3",borderRadius:10,padding:"12px 14px",border:"1px solid #e8dfd2"}}>
          <div style={{fontSize:12,fontWeight:700,color:"#2b2622",marginBottom:8}}>Estado</div>
          <div style={{fontSize:11,color: status==="error" ? "#991b1b" : "#574e44",lineHeight:1.7}}>
      ...
      <div ref={mapNodeRef} style={{height:420,borderRadius:12,overflow:"hidden",border:`2px solid ${measureMode?"#2b2622":"#e8dfd2"}`,marginBottom:12}} />
      ...
      {draft && (
        <div style={{background:"#fffbeb",border:"2px solid #b54708",borderRadius:10,padding:16,marginBottom:12}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr auto",gap:10,alignItems:"end"}}>
            <div><LBL>Metros calculados</LBL><input value={draft.ml} readOnly style={{...SI,background:"#f4eee4",fontWeight:700,color:"#574e44"}} /></div>
            ...
            <div style={{display:"flex",gap:6}}>
              <button onClick={confirmDraft} style={B("#2b2622","#ffffff")}>Agregar</button>
              <button onClick={()=>{clearTemp(); setDraft(null);}} style={B("#f9e9e4","#cc0000")}>Cerrar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {(measurements||[]).length>0 && (
        <div style={{marginBottom:12}}>
          <div style={{fontSize:12,fontWeight:600,color:"#574e44",marginBottom:8}}>Tramos medidos:</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {(measurements||[]).map(seg=>(
              <div key={seg.id} style={{background:"#fbf8f3",borderRadius:8,padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,border:"1px solid #e8dfd2"}}>
                <div>
                  <div style={{fontSize:12,fontWeight:700,color:"#2b2622"}}>{seg.label}</div>
                  <div style={{fontSize:10,color:"#756a5e"}}>{seg.tipo} · {Number(seg.ml||0).toFixed(2)} m</div>
                </div>
                <button onClick={()=>removeMeasurement(seg.id)} style={{background:"#f9e9e4",border:"none",color:"#cc0000",borderRadius:5,padding:"3px 8px",cursor:"pointer",fontSize:11}}>×</button>
              </div>
            ))}
          </div>
        </div>
