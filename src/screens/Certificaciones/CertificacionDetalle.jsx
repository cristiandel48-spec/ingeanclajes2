  ...
  return(
    <div>
      <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap"}}>
        <button style={B("#f2f4f7","#475467")} onClick={onVolver}>Volver</button>
        {typeof onEditar==="function" && <button style={{...B("#f2f4f7","#475467")}} onClick={()=>onEditar(cert)}>Editar</button>}
        <button style={{...B("#101828"),opacity:generando?0.65:1}} disabled={generando} onClick={descargar}>
          {generando ? "Generando…" : "Descargar PDF"}
        </button>
        {typeof onImprimir==="function" && <button style={B("#f2f4f7","#475467")} onClick={()=>onImprimir(cert)}>Imprimir</button>}
      </div>
      <div style={{background:"#f2f4f7",border:"1px solid #fed7aa",borderRadius:12,padding:"12px 14px",marginBottom:16}}>
        <div style={{fontSize:11,fontWeight:700,color:"#475467",textTransform:"uppercase",letterSpacing:0.7}}>Certificación</div>
