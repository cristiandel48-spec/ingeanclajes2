  ...
  return(
    <div>
      <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap"}}>
        <button style={B("#f4eee4","#574e44")} onClick={onVolver}>Volver</button>
        {typeof onEditar==="function" && <button style={{...B("#f4eee4","#574e44")}} onClick={()=>onEditar(cert)}>Editar</button>}
        <button style={{...B("#2b2622"),opacity:generando?0.65:1}} disabled={generando} onClick={descargar}>
          {generando ? "Generando…" : "Descargar PDF"}
        </button>
        {typeof onImprimir==="function" && <button style={B("#f4eee4","#574e44")} onClick={()=>onImprimir(cert)}>Imprimir</button>}
      </div>
      <div style={{background:"#f4eee4",border:"1px solid #fed7aa",borderRadius:12,padding:"12px 14px",marginBottom:16}}>
        <div style={{fontSize:11,fontWeight:700,color:"#574e44",textTransform:"uppercase",letterSpacing:0.7}}>Certificación</div>
