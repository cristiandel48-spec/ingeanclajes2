  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #cc0000", borderRadius: 14, padding: 20, marginBottom: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, gap: 10 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: "#101828" }}>🎤 Armar cotización hablando</div>
        <button onClick={onCerrar} style={{ ...B("#f2f4f7", "#475467"), fontSize: 12, padding: "6px 12px" }}>Cerrar</button>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        <button onClick={interpretar} disabled={cargando || !texto.trim()} style={{ ...B("#101828"), opacity: cargando || !texto.trim() ? 0.6 : 1 }}>
          {cargando ? "Interpretando…" : "Interpretar lo dictado"}
        </button>
        {texto && !cargando && (
          <button onClick={() => { setTexto(""); setPropuesta(null); setError(""); }} style={{ ...B("#f2f4f7", "#475467"), fontSize: 12 }}>
      ...
      {error && (
        <div style={{ background: "#FEF3F2", border: "1px solid #FECDCA", color: "#cc0000", borderRadius: 10, padding: "11px 14px", fontSize: 12.5, marginTop: 12, lineHeight: 1.5 }}>
          {error}
        </div>
      )}

      {propuesta && (
        <div style={{ background: "#fafafa", border: "1px solid #eaecf0", borderRadius: 12, padding: 16, marginTop: 14 }}>
          <div style={{ fontSize: 12.5, fontWeight: 700, color: "#101828", marginBottom: 10 }}>Esto es lo que se entendió</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 8, marginBottom: 12 }}>
            {[
              ...
              <div key={etiqueta} style={{ background: "#fff", borderRadius: 8, padding: "8px 10px", border: "1px solid #f2f4f7" }}>
                <div style={{ fontSize: 9.5, color: "#98a2b3", textTransform: "uppercase", marginBottom: 2 }}>{etiqueta}</div>
                <div style={{ fontSize: 12, fontWeight: 600, color: valor ? "#101828" : "#d0d5dd" }}>{valor || "no se dijo"}</div>
              </div>
            ))}
          </div>

          {propuesta.alcance && (
            <div style={{ background: "#fff", borderRadius: 8, padding: "10px 12px", border: "1px solid #f2f4f7", marginBottom: 12 }}>
              <div style={{ fontSize: 9.5, color: "#98a2b3", textTransform: "uppercase", marginBottom: 3 }}>Descripción propuesta</div>
              <div style={{ fontSize: 12, color: "#475467", lineHeight: 1.55 }}>{propuesta.alcance}</div>
            </div>
          )}
          ...
          {propuesta.items.length === 0 ? (
            <div style={{ background: "#f2f4f7", border: "1px solid #eaecf0", color: "#B54708", borderRadius: 8, padding: "10px 12px", fontSize: 12 }}>
              No se reconoció ningún servicio del catálogo. Puedes aplicar los datos del cliente y
              agregar los ítems a mano.
            </div>
          ) : (
            <div style={{ background: "#fff", borderRadius: 8, border: "1px solid #f2f4f7", overflow: "hidden" }}>
              {propuesta.items.map((item, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "8px 12px", borderBottom: i < propuesta.items.length - 1 ? "1px solid #fafafa" : "none", fontSize: 12 }}>
                  ...
                  <span style={{ color: "#667085", whiteSpace: "nowrap" }}>{item.cant} {item.unit} · {fmt(item.vu * item.cant)}</span>
                </div>
              ))}
              <div style={{ display: "flex", justifyContent: "space-between", padding: "9px 12px", background: "#f2f4f7", fontSize: 12.5, fontWeight: 700 }}>
                <span style={{ color: "#667085" }}>Subtotal estimado</span>
          ...
          {propuesta.avisos.length > 0 && (
            <div style={{ background: "#f2f4f7", border: "1px solid #eaecf0", borderRadius: 8, padding: "10px 12px", fontSize: 11.5, color: "#B54708", marginTop: 10, lineHeight: 1.55 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
            <button onClick={() => onAplicar(propuesta)} style={B("#101828", "#ffffff")}>
              Aplicar a la cotización
            </button>
            <button onClick={() => setPropuesta(null)} style={{ ...B("#f2f4f7", "#475467"), fontSize: 12 }}>
              Descartar
            </button>
            <div style={{ fontSize: 10.5, color: "#98a2b3", alignSelf: "center", flex: "1 1 200px" }}>
