...
export default function DictarCotizacion({ onAplicar, onCerrar }) {
  ...
  return (
    <div style={{ background: "var(--surface, #fff)", border: "1px solid #cc0000", borderRadius: 14, padding: 20, marginBottom: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, gap: 10 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-main, #1a1a2e)" }}>🎤 Armar cotización hablando</div>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {texto && !cargando && (
          <button onClick={() => { setTexto(""); setPropuesta(null); setError(""); }} style={{ ...B("var(--btn-cancelar-bg, #f1f5f9)", "var(--btn-cancelar-txt, #475569)"), fontSize: 12 }}>
      ...
      {propuesta && (
        <div style={{ background: "var(--surface-subtle, #f8fafc)", border: "1px solid var(--border, #e2e8f0)", borderRadius: 12, padding: 16, marginTop: 14 }}>
          <div style={{ fontSize: 12.5, fontWeight: 700, color: "var(--text-main, #1a1a2e)", marginBottom: 10 }}>Esto es lo que se entendió</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 8, marginBottom: 12 }}>
            {[
              ...
              <div key={etiqueta} style={{ background: "var(--surface, #fff)", borderRadius: 8, padding: "8px 10px", border: "1px solid var(--border, #f1f5f9)" }}>
                <div style={{ fontSize: 9.5, color: "var(--text-subtle, #94a3b8)", textTransform: "uppercase", marginBottom: 2 }}>{etiqueta}</div>
                <div style={{ fontSize: 12, fontWeight: 600, color: valor ? "var(--text-main, #1a1a2e)" : "var(--text-subtle, #cbd5e1)" }}>{valor || "no se dijo"}</div>
              </div>
            ))}
          </div>

          {propuesta.alcance && (
            <div style={{ background: "var(--surface, #fff)", borderRadius: 8, padding: "10px 12px", border: "1px solid var(--border, #f1f5f9)", marginBottom: 12 }}>
              <div style={{ fontSize: 9.5, color: "var(--text-subtle, #94a3b8)", textTransform: "uppercase", marginBottom: 3 }}>Descripción propuesta</div>
              <div style={{ fontSize: 12, color: "var(--text-main, #475569)", lineHeight: 1.55 }}>{propuesta.alcance}</div>
            </div>
          )}
          ...
          {propuesta.items.length === 0 ? (
            <div style={{ background: "rgba(245, 158, 11, 0.15)", border: "1px solid rgba(253, 227, 196, 0.3)", color: "#fbbf24", borderRadius: 8, padding: "10px 12px", fontSize: 12 }}>
              No se reconoció ningún servicio del catálogo. Puedes aplicar los datos del cliente y
              agregar los ítems a mano.
            </div>
          ) : (
            <div style={{ background: "var(--surface, #fff)", borderRadius: 8, border: "1px solid var(--border, #f1f5f9)", overflow: "hidden" }}>
              {propuesta.items.map((item, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "8px 12px", borderBottom: i < propuesta.items.length - 1 ? "1px solid var(--border, #f8fafc)" : "none", fontSize: 12 }}>
                  <span style={{ color: "var(--text-main, #1a1a2e)" }}>
                    {item.desc}
                    {/* Se avisa cuando el precio no es el de siempre: un valor
                        dictado por error se veria igual que uno correcto. */}
                    {item.precioDictado && (
                      <span style={{ color: "#f59e0b", fontSize: 10.5, display: "block", marginTop: 2 }}>
                        precio dictado {fmt(item.vu)} · en catálogo {fmt(item.vuCatalogo)}
                      </span>
                    )}
                  </span>
                  <span style={{ color: "var(--text-muted, #64748b)", whiteSpace: "nowrap" }}>{item.cant} {item.unit} · {fmt(item.vu * item.cant)}</span>
                </div>
              ))}
              <div style={{ display: "flex", justifyContent: "space-between", padding: "9px 12px", background: "var(--surface-subtle, #f1f5f9)", fontSize: 12.5, fontWeight: 700 }}>
                <span style={{ color: "var(--text-muted, #64748b)" }}>Subtotal estimado</span>
          ...
          {propuesta.avisos.length > 0 && (
            <div style={{ background: "rgba(245, 158, 11, 0.15)", border: "1px solid rgba(253, 227, 196, 0.3)", borderRadius: 8, padding: "10px 12px", fontSize: 11.5, color: "#fbbf24", marginTop: 10, lineHeight: 1.55 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
            ...
            <button onClick={() => setPropuesta(null)} style={{ ...B("var(--btn-cancelar-bg, #f1f5f9)", "var(--btn-cancelar-txt, #475569)"), fontSize: 12 }}>
              Descartar
            </button>
            <div style={{ fontSize: 10.5, color: "var(--text-subtle, #94a3b8)", alignSelf: "center", flex: "1 1 200px" }}>
