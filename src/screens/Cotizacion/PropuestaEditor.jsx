...
export default function PropuestaEditor({
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #e8dfd2", borderRadius: 12, padding: 20, marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginBottom: 18, paddingBottom: 12, borderBottom: "2px solid #2b2622" }}>
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#2b2622", textTransform: "uppercase", letterSpacing: 1 }}>
            Propuesta {indice + 1} de {totalPropuestas}
          </div>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#2b2622", marginTop: 2 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {totalPropuestas > 1 && (
            <button
              ...
              onClick={onEliminar}
              style={{ background: "#f9e9e4", border: "none", color: "#cc0000", borderRadius: 6, width: 26, height: 26, cursor: "pointer", fontSize: 15, lineHeight: 1 }}
            >×</button>
          )}
        </div>
      </div>
          ...
          <div>
            ...
            <div style={{display:"flex",gap:8}}>
              {[["linea_vida","Línea de vida"],["puntos_anclaje","Puntos de anclaje"],["obra_blanca","Obra blanca"]].map(([v,l])=>(
                <button key={v} onClick={()=>set("tipoCotizacion", v)} style={{...B(p.tipoCotizacion===v?"#2b2622":"#2b2622",p.tipoCotizacion===v?"#fff":"#a2988a"),flex:1,justifyContent:"center",border:"2px solid "+(p.tipoCotizacion===v?"#2b2622":"#e8dfd2"),fontSize:11,fontWeight:700,padding:"7px 4px"}}>{l}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        <div style={{marginBottom:18}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <span style={{fontSize:12,fontWeight:600,color:"#2b2622"}}>Fotos de la propuesta</span>
            <span style={{fontSize:10,color:"#a2988a"}}>Se imprimen en el PDF</span>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
            {fotos.map((f,i)=>(
              <div key={f.id} style={{borderRadius:8,overflow:"hidden",border:"1px solid #e8dfd2",background:"#fbf8f3"}}>
                ...
                <div style={{padding:"6px 8px",display:"flex",gap:4,alignItems:"center"}}>
                  ...
                  <button onClick={()=>setFotos(prev=>prev.filter(item=>item.id!==f.id))} style={{background:"#f9e9e4",border:"none",color:"#cc0000",borderRadius:6,width:22,height:22,cursor:"pointer",fontSize:14,flexShrink:0,lineHeight:1}}>×</button>
                </div>
              </div>
            ))}
            <div onClick={()=>fotosRef.current.click()} style={{border:"2px dashed #2b2622",borderRadius:10,minHeight:140,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",cursor:"pointer",background:"#f4eee4",color:"#2b2622",fontWeight:600,gap:6}}>
        ...
        <div style={{marginBottom:18}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <span style={{fontSize:12,fontWeight:600,color:"#2b2622"}}>Medición automática con Google Maps</span>
            <div style={{display:"flex",gap:8}}>
              <button
                type="button"
                onClick={()=>set("medicionAutomatica", !medicionActiva)}
                style={{...B(medicionActiva?"#027a48":"#f4eee4",medicionActiva?"#ecfdf5":"#574e44"),fontSize:11,padding:"5px 12px",border:"1px solid " + (medicionActiva?"#027a48":"#93c5fd")}}
              >
              ...
              {autoMapImg && (
                <button
                  ...
                  style={{...B("#f4eee4","#574e44"),fontSize:11,padding:"5px 12px",border:"1px solid #fdba74"}}
                >
          ...
          <div style={{fontSize:11,color:"#756a5e",marginBottom:10}}>
            El mapa y los tramos medidos pertenecen solo a esta propuesta.
          </div>
          {medicionActiva && !mapaHabilitado ? (
            <div style={{background:"#fbf8f3",border:"1px dashed #d9cdbb",borderRadius:12,padding:"18px 16px",fontSize:12,color:"#756a5e",textAlign:"center"}}>
              Esta propuesta tiene medición activada.{" "}
              <button type="button" onClick={onPedirMapa} style={{...B("#f4eee4","#574e44"),fontSize:11,padding:"4px 12px",marginLeft:6}}>Abrir su mapa</button>
              <div style={{marginTop:6,fontSize:10.5,color:"#a2988a"}}>Se muestra un mapa a la vez para no cargar la pantalla.</div>
            </div>
          ) : medicionActiva ? (
            <div style={{background:"#fbf8f3",border:"1px solid #e8dfd2",borderRadius:12,padding:14}}>
        ...
        <div style={{marginBottom:18}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <span style={{fontSize:12,fontWeight:600,color:"#2b2622"}}>Detalle económico</span>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{const nuevos=measurementsToQuoteItems(p.geoMediciones);setItems(nuevos.map((item,index)=>({...item,id:index+1})));}} style={{...B("#f4eee4","#574e44"),fontSize:11,padding:"5px 12px"}}>Jalar mediciones</button>
              <button onClick={()=>setShowDB(!showDB)} style={{...B(showDB?"#e8dfd2":"transparent","#2b2622"),border:"1px solid #e8dfd2",fontSize:11,padding:"5px 12px"}}>{showDB?"Cerrar catálogo":"Catálogo"}</button>
            </div>
          </div>

          {showDB&&(
            <div style={{background:"#fbf8f3",borderRadius:10,padding:16,marginBottom:14,border:"1px solid #e8dfd2"}}>
              <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>{ITEMS_DB.map((cat,i)=><button key={i} onClick={()=>setDbCat(i)} style={{...B(dbCat===i?"#2b2622":"#2b2622",dbCat===i?"#fff":"#a2988a"),border:`1px solid ${dbCat===i?"#2b2622":"#e8dfd2"}`,fontSize:11,padding:"5px 12px"}}>{cat.categoria}</button>)}</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{ITEMS_DB[dbCat].items.map((it,i)=><div key={i} style={{background:"#f4eee4",borderRadius:8,padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:"#2b2622",marginBottom:2}}>{it.desc}</div><div style={{fontSize:11,color:"#574e44"}}>{it.unit} · {fmt(it.vu)}</div></div><button onClick={()=>{setItems(prev=>[...prev,{id:siguienteId(prev),desc:it.desc,cant:1,unit:it.unit,vu:it.vu}]);}} style={{...B("#2b2622"),padding:"5px 12px",fontSize:12,flexShrink:0}}>+</button></div>)}</div>
            </div>
          )}

          {/* Tabla de ítems. La clase "tabla-items" evita que en el celular se
              colapse a una columna: conserva sus columnas y se desplaza. */}
          <div className="tabla-items" style={{border:"1px solid #e8dfd2",borderRadius:10,overflow:"hidden",marginBottom:10}}>
            <div style={{display:"grid",gridTemplateColumns:"3fr 0.65fr 0.75fr 1.1fr 1.1fr 28px",background:"#1a2840",color:"#a2988a",fontSize:10,textTransform:"uppercase",padding:"9px 12px",letterSpacing:0.5}}>
              <span>Descripción</span><span>Cant.</span><span>Unidad</span><span>Valor unit.</span><span style={{textAlign:"right"}}>Subtotal</span><span/>
            </div>
            {p.items.map((it,idx)=>(
              <div key={it.id} style={{display:"grid",gridTemplateColumns:"3fr 0.65fr 0.75fr 1.1fr 1.1fr 28px",alignItems:"center",padding:"5px 10px",background:idx%2===0?"#fbf8f3":"#fff",borderTop:"1px solid #f4eee4"}}>
                ...
                <div style={{textAlign:"right",fontSize:12,fontWeight:600,color:"#2b2622",paddingRight:4}}>{fmt(it.cant*it.vu)}</div>
            ...
            {p.items.length===0&&<div style={{padding:"18px 12px",textAlign:"center",fontSize:12,color:"#a2988a"}}>Sin ítems — agrega desde catálogo o manualmente</div>}
          </div>

          <button onClick={()=>{setItems(prev=>[...prev,{id:siguienteId(prev),desc:"",cant:1,unit:"ML",vu:0}]);}} style={{...B("#f4eee4","#2b2622"),border:"1px dashed #cc0000",width:"100%",justifyContent:"center",marginBottom:16,fontSize:12}}>+ Agregar ítem manual</button>

          {/* Tabla de totales */}
          <div style={{border:"1px solid #e8dfd2",borderRadius:10,overflow:"hidden"}}>
            {[["SUBTOTAL",sub],["ADMINISTRACIÓN",0],["IMPREVISTOS",0],["UTILIDAD "+p.util+"%",ut],["IVA SOBRE LA UTILIDAD (19%)",iva]].map(([lbl,v])=>(
              <div key={lbl} style={{display:"flex",justifyContent:"space-between",padding:"9px 14px",borderBottom:"1px solid #f4eee4",fontSize:12,color:"#574e44"}}>
                <span>{lbl}</span><span style={{fontWeight:500,color:"#2b2622"}}>{v?fmt(v):"$  -"}</span>
              </div>
            ))}
            <div style={{display:"flex",justifyContent:"space-between",padding:"12px 14px",background:"#1a2840"}}>
              ...
              <span style={{fontSize:15,fontWeight:700,color:"#2b2622"}}>{fmt(tot)}</span>
            </div>
            <div style={{padding:"6px 14px",fontSize:10,color:"#a2988a",textAlign:"center",background:"#fbf8f3"}}>EL IVA ES EL 19% DE LA UTILIDAD</div>
          </div>
          ...
          <div style={{marginTop:12,display:"grid",gridTemplateColumns:"120px 1fr",gap:12,alignItems:"end"}}>
            ...
            <div style={{fontSize:11,color:"#756a5e",paddingBottom:10}}>Ajusta el porcentaje de utilidad para recalcular el total</div>
          </div>
        </div>
        ...
        <div style={{paddingTop:12,borderTop:"1px solid #f4eee4",marginTop:4}}>
          <div style={{fontSize:11,fontWeight:700,color:"#2b2622",marginBottom:2}}>Condiciones comerciales</div>
          <div style={{fontSize:10.5,color:"#a2988a",marginBottom:10}}>Pertenecen a esta propuesta, pero en el documento salen al final, en el cierre.</div>
        </div>
        ...
        <div style={{marginTop:18}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,marginBottom:2}}>
            <div style={{fontSize:11,fontWeight:700,color:"#2b2622"}}>Esta cotización incluye</div>
            <button
              ...
              style={{
                ...
                color:p.incluyeTexto===DEFAULT_COT_INCLUYE?"#d9cdbb":"#2b2622",
          ...
          <div style={{fontSize:10.5,color:"#a2988a",marginBottom:10}}>
