      ...
      <div id="pz">
        ...
        <div style={{...CD,marginBottom:20}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            <thead><tr style={{background:"var(--surface-subtle, #f1f5f9)"}}>{["Obra","Cliente","Ingresos","Cobrado","Saldo","Costos Dir.","Util. Bruta","Margen %","Estado"].map(h=><th key={h} style={{padding:"9px 12px",textAlign:["Ingresos","Cobrado","Saldo","Costos Dir.","Util. Bruta","Margen %"].includes(h)?"right":"left",color:"var(--text-muted, #64748b)",fontWeight:500,fontSize:11}}>{h}</th>)}</tr></thead>
            <tbody>
              {sel.map((o,i)=>(
                <tr key={o.id} style={{borderBottom:"1px solid var(--border, #e2e8f0)",background:i%2===0?"var(--surface, #ffffff)":"var(--surface-subtle, #f8fafc)"}}>
                  <td style={{padding:"10px 12px",color:"#60b4ff",fontSize:11}}>{o.id}</td>
                  <td style={{padding:"10px 12px",fontSize:12,fontWeight:500,color:"var(--text-main, #0f172a)"}}>{o.cliente}</td>
              ...
              <tr style={{background:"var(--surface-subtle, #f1f5f9)",fontWeight:700}}>
            ...
            {sel.map(o=>(
              <div key={o.id} style={{marginBottom:14}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}>
                  <span style={{color:"var(--text-main, #1a1a2e)",fontWeight:500}}>{o.cliente}</span>
                  <span style={{color:o.margenBruto>25?"#4ade80":o.margenBruto>10?"#f5c842":"#ef4444",fontWeight:700}}>{o.margenBruto}%</span>
                </div>
                <div style={{height:8,background:"var(--border, #e2e8f0)",borderRadius:4}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"var(--text-muted, #64748b)",marginTop:3}}>
          ...
          <div style={CD}>
            ...
            <div style={{marginBottom:14}}>
              {[["Costos proveedores",totCost,"#fb923c"],["Nómina mensual estimada",nomMes,"#c084fc"],["Saldo pendiente cobrar",totIng-totCob,"#f5c842"],["Utilidad bruta estimada",totUtil,"#4ade80"]].map(([k,v,c])=>(
                <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"10px 0",borderBottom:"1px solid var(--border, #e2e8f0)",fontSize:13}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:10,height:10,borderRadius:2,background:c,flexShrink:0}}/><span style={{color:"var(--text-muted, #475569)"}}>{k}</span></div>
                  <span style={{fontWeight:700,color:c}}>{fmt(v)}</span>
                </div>
              ))}
            </div>
            <div style={{background:"var(--surface-subtle, #f1f5f9)",borderRadius:8,padding:"12px 14px",fontSize:12,color:"var(--text-muted, #64748b)",lineHeight:1.7,border:"1px solid var(--border, #e2e8f0)"}}>
              <div style={{fontWeight:600,color:"var(--text-main, #1a1a2e)",marginBottom:6}}>Indicadores clave</div>
