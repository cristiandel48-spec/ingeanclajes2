...
export default function Nomina({ctx}){
  ...
  const renderNominaMetricCard=({label,value,color="#2b2622",hint="",span=1})=>(
    <div style={{background:"#fff",borderRadius:8,padding:"10px 12px",gridColumn:span>1?("span " + span):undefined}}>
      <div style={{fontSize:10,color:"#756a5e"}}>{label}</div>
      <div style={{fontWeight:700,color,fontSize:span>1?16:15,marginTop:3}}>{value}</div>
      {hint ? <div style={{fontSize:10,color:"#a2988a",marginTop:4,lineHeight:1.4}}>{hint}</div> : null}
    </div>
  );
    ...
    return(
      <div style={{background:"#fbf8f3",border:"1px solid #e8dfd2",borderRadius:12,padding:"14px 16px"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontWeight:700,color:"#2b2622"}}>{empleado.nombre}</div>
            <div style={{fontSize:11,color:"#756a5e"}}>{subtitle}</div>
          </div>
  ...
  return(
    <div style={{padding:28}}>
      <H1 title="Nómina y Empleados" subtitle="Proceso quincenal: preparar el corte, revisar novedades y generar el pago"
        action={<button style={B("#2b2622")} onClick={()=>setTab("nuevo")}>+ Nuevo Empleado</button>}/>
        ...
        >
          ...
          <div style={{marginTop:7,display:"flex",flexWrap:"wrap",gap:8}}>
            {pendientesRevision.map((e)=>(
              <span key={e.id} style={{background:"#fff",border:"1px solid #e8dfd2",borderRadius:6,padding:"4px 4px 4px 9px",fontSize:11.5,color:"#B54708",display:"inline-flex",alignItems:"center",gap:7}}>
      ...
      {tab==="nuevo"&&(
        <div style={{display:"grid",gap:16,maxWidth:980}}>
          <div style={{background:"#ffffff",border:"1px solid #e8dfd2",borderRadius:12,padding:20,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
            <div style={{fontSize:11,fontWeight:700,color:"#2b2622",textTransform:"uppercase",letterSpacing:1,marginBottom:16,borderBottom:"1px solid #e8dfd2",paddingBottom:10}}>
              Catálogo de cargos
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1.1fr 1.4fr auto",gap:12,alignItems:"end"}}>
              ...
              <button onClick={guardarCargo} style={{...B("#2b2622"),justifyContent:"center"}}>Guardar cargo</button>
            </div>
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:14}}>
              {cargosDisponibles.map((cargo)=>(
                <span key={cargo} style={{background:nf.cargo===cargo?"#f4eee4":"#f4eee4",color:nf.cargo===cargo?"#b54708":"#574e44",border:"1px solid " + (nf.cargo===cargo?"#f59e0b":"#d9cdbb"),borderRadius:999,padding:"6px 10px",fontSize:11,fontWeight:600,cursor:"pointer"}} onClick={()=>setNf({...nf,cargo:cargo})}>
                  {cargo}
                </span>
              ))}
            </div>
          ...
          <div style={{background:"#ffffff",border:"1px solid #e8dfd2",borderRadius:12,padding:24,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
            <div style={{fontSize:11,fontWeight:700,color:"#2b2622",textTransform:"uppercase",letterSpacing:1,marginBottom:18,borderBottom:"2px solid #cc000022",paddingBottom:10}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
              ...
              <div style={{background:"#fbf8f3",borderRadius:8,padding:"10px 12px",fontSize:12,color:"#574e44",display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:18}}>💵</span>
                <div>
                  <div style={{fontSize:10,color:"#a2988a"}}>Salario ingresado</div>
                  <div style={{fontWeight:700,color:"#2b2622",fontSize:14}}>{fmt(nf.salario)}</div>
                  <div style={{fontSize:10,color:"#756a5e",marginTop:2}}>
            ...
            <div style={{background:"#fbf8f3",borderRadius:10,padding:16,marginBottom:16,border:"1px solid #e8dfd2"}}>
              <div style={{fontSize:11,fontWeight:700,color:"#574e44",textTransform:"uppercase",marginBottom:12,letterSpacing:0.5}}>🏦 Datos Bancarios</div>
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={guardarNuevoEmp} style={B("#2b2622")}>✅ Guardar Empleado</button>
              <button onClick={()=>setTab("lista")} style={B("#f4eee4","#574e44")}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={CD}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {empleadosBase.map((e,idx)=>{
                ...
                return(
                <div key={e.id} style={{background:"#f4eee4",borderRadius:10,padding:"14px 16px",border:selId===e.id?"1px solid #cc0000":"1px solid #e8dfd2",cursor:"pointer"}} onClick={()=>setSelId(selId===e.id?null:e.id)}>
                  <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
                    <Av init={e.avatar} color={PAL[idx%PAL.length]} size={38}/>
                    <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{e.nombre}</div><div style={{fontSize:11,color:"#574e44"}}>{e.cargo}</div></div>
                    <div style={{textAlign:"right"}}><div style={{fontSize:13,fontWeight:700,color:"#2b2622"}}>{fmt(e.salario)}</div></div>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,fontSize:11,color:"#756a5e",marginBottom:6}}>
                  ...
                  {(()=>{
                    ...
                    return pf.diasTrabajados>0 ? (
                      <div style={{background:"#f0fdf4",borderRadius:6,padding:"7px 10px",fontSize:11,marginBottom:6,display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>
                        <div><span style={{color:"#756a5e"}}>📅 Ingreso: </span><strong>{e.fechaIngreso||"N/A"}</strong></div>
                        <div><span style={{color:"#756a5e"}}>Contrato: </span><strong>{TIPOS_CONTRATO_LABELS[e.tipoContrato]||e.tipoContrato||"Sin definir"}</strong></div>
                        <div><span style={{color:"#756a5e"}}>✈️ Vacac.: </span><strong style={{color:"#027a48"}}>{vacacionesPendientes.dias}d</strong></div>
                        <div><span style={{color:"#756a5e"}}>Días corte: </span><strong style={{color:"#574e44"}}>{resumen.diasNomina}</strong></div>
                      </div>
                    ) : null;
                  })()}
                  <div style={{background:"#fbf8f3",borderRadius:6,padding:"7px 10px",fontSize:11,marginBottom:6}}>
                    <div style={{color:"#756a5e",marginBottom:2}}>🏦 Datos bancarios</div>
                    <div style={{color:"#2b2622"}}>{e.banco||"-"} · {e.tipoCuenta||"-"}</div>
                    <div style={{color:"#574e44",fontFamily:"monospace"}}>{e.numeroCuenta||"-"}</div>
                  </div>
                  {selId===e.id&&(
                    <div style={{marginTop:10,borderTop:"1px solid rgba(255,255,255,0.2)",paddingTop:10}} onClick={(event)=>event.stopPropagation()}>
                      <div style={{fontSize:10,color:"#2b2622",fontWeight:600,textTransform:"uppercase",marginBottom:8}}>Datos del empleado</div>
                      {editEmpId!==e.id&&<button onClick={()=>{setEditEmpId(e.id);setEditEmpData({...e});}} style={{...B("#e8dfd2","#60b4ff"),fontSize:11,width:"100%",justifyContent:"center",marginBottom:10}}>✏️ Editar datos del empleado</button>}
                      {editEmpId===e.id&&editEmpData&&(
                        <div>
                          ...
                          <div style={{display:"flex",gap:8,marginBottom:10}}>
                            <button onClick={()=>{actualizarEmpleado(e.id,editEmpData);setEditEmpId(null);setEditEmpData(null);}} style={{...B("#027a48","#2b2622"),fontSize:11,flex:1,justifyContent:"center"}}>💾 Guardar cambios</button>
                            <button onClick={()=>{setEditEmpId(null);setEditEmpData(null);}} style={{...B("#f9e9e4","#cc0000"),fontSize:11,justifyContent:"center"}}>✕ Cancelar</button>
                          </div>
                        </div>
                      )}
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
                        {[["Días laborados",resumen.diasNomina,"#574e44"],["Días incapacidad",resumen.diasIncapacidad,"#cc0000"],["Salario corte",fmt(resumen.salario),"#2b2622"],["Incapacidad",fmt(resumen.incapacidadTotal),"#027a48"],["Aux. transp.",fmt(resumen.auxilioTransporte),"#60b4ff"],["H. extras",fmt(resumen.horasExtras),"#b54708"],["Comisiones",fmt(resumen.comisiones),"#c084fc"],["Salud 4%",fmt(resumen.salud),"#cc0000"],["Pensión 4%",fmt(resumen.pension),"#fb7185"],["Neto",fmt(resumen.neto),"#2b2622"]].map(([k,v,c])=>(
                          <div key={k} style={{background:"#ffffff",borderRadius:6,padding:"8px 10px"}}><div style={{fontSize:9,color:"#756a5e",marginBottom:2}}>{k}</div><div style={{fontSize:11,fontWeight:700,color:c}}>{v}</div></div>
                        ))}
                      </div>
                      <div style={{background:"#ffffff",borderRadius:8,padding:"10px 12px",marginBottom:10,border:"1px solid #e8dfd2"}}>
                        <div style={{fontSize:10,color:"#2b2622",fontWeight:700,textTransform:"uppercase",marginBottom:8}}>Deducciones personalizadas</div>
                        {(e.deduccionesPersonalizadas||[]).length>0 ? (
                          <div style={{display:"grid",gap:6,marginBottom:10}}>
                            {(e.deduccionesPersonalizadas||[]).map((deduccion)=>(
                              <div key={deduccion.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#fbf8f3",borderRadius:6,padding:"8px 10px",fontSize:11}}>
                                <div><div style={{fontWeight:600,color:"#2b2622"}}>{deduccion.nombre}</div><div style={{color:"#756a5e"}}>{fmt(deduccion.valor)} mensual</div></div>
                                <button type="button" onClick={()=>quitarDeduccion(e.id,deduccion.id)} style={{...B("#f9e9e4","#cc0000"),border:"1px solid #fecaca",padding:"6px 10px",fontSize:11}}>Quitar</button>
                              </div>
                            ))}
                          </div>
                        ) : (
                          <div style={{fontSize:11,color:"#756a5e",marginBottom:10}}>Sin deducciones adicionales.</div>
                        )}
                        <div style={{display:"grid",gridTemplateColumns:"1.3fr 1fr auto",gap:8}}>
                          ...
                          <button type="button" onClick={agregarDeduccion} style={{...B("#2b2622"),justifyContent:"center",alignSelf:"end"}}>Agregar</button>
                        </div>
                      </div>
                      <button onClick={()=>{setShowHE(e.id);setTab("he");}} style={{...B("#2b2622","#b54708"),border:"1px solid #7a6610",fontSize:11,width:"100%",justifyContent:"center"}}>➕ Agregar horas extras / comisiones</button>
                    </div>
                  )}
                </div>
              )})}
      ...
      {tab==="prestaciones"&&(
        <div>
          <div style={{...CD,maxWidth:980,margin:"0 auto"}}>
            ...
            <div style={{background:"#fbf8f3",border:"1px solid #e8dfd2",borderRadius:12,padding:"12px 14px",fontSize:12,color:"#574e44",lineHeight:1.7,marginBottom:16}}>
            ...
            {!empleadoPrestacionActivo ? (
              <div style={{fontSize:12,color:"#a2988a",textAlign:"center",padding:"18px 0"}}>No hay empleados activos para liquidar prestaciones.</div>
            ) : (
              <>
                {renderNominaEmpleadoCard({
                  ...
                  metrics:[
                    {label:"Fecha causación", value:prestacionPreview?.fechaCausacion ? fmtD(prestacionPreview.fechaCausacion) : "—", color:"#2b2622"},
                    {label:"Fecha límite", value:prestacionPreview?.fechaLimite ? fmtD(prestacionPreview.fechaLimite) : "—", color:"#b54708"},
                    {label:"Estado sugerido", value:PRESTACION_ESTADOS_LABELS[prestacionPreview?.estado || "provisionada"], color:"#574e44"},
                  ],
                })}
                ...
                <div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:12,padding:"14px 16px",marginBottom:16}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap",marginBottom:10}}>
                    <div>
                      ...
                      <div style={{fontSize:12,color:"#574e44",marginTop:4}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                    <div style={{background:"#fff",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Tipo</div><div style={{fontWeight:700,color:"#2b2622",marginTop:4}}>{PRESTACION_TIPOS_LABELS[prestacionTipo]}</div></div>
                    <div style={{background:"#fff",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Periodo</div><div style={{fontWeight:700,color:"#2b2622",marginTop:4}}>{prestacionPreview?.periodoLabel || "—"}</div></div>
                    <div style={{background:"#fff",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Días trabajados</div><div style={{fontWeight:700,color:"#2b2622",marginTop:4}}>{prestacionPreview?.diasLiquidados || 0}</div></div>
                    <div style={{background:"#fff",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Valor provisionado</div><div style={{fontWeight:700,color:(prestacionPreview?.valor || 0)>0?"#027a48":"#a2988a",marginTop:4}}>{fmt(prestacionPreview?.valor || 0)}</div></div>
                  </div>
                </div>

                <div style={{background:"#fff",border:"1px solid #e8dfd2",borderRadius:12,padding:"14px 16px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:12,flexWrap:"wrap"}}>
                    <div>
                      <div style={{fontSize:11,fontWeight:700,color:"#2b2622",textTransform:"uppercase",letterSpacing:0.7}}>Historial prestacional</div>
                      <div style={{fontSize:12,color:"#756a5e",marginTop:4}}>Consulta, paga o consigna prestaciones del empleado seleccionado.</div>
                    </div>
                    <div style={{fontSize:12,color:"#756a5e"}}>{historialPrestacionesEmpleado.length} registro(s)</div>
                  </div>
                  <div style={{overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:920}}>
                      <thead>
                        <tr style={{background:"#fbf8f3"}}>
                          {["Concepto","Periodo","Valor","Causación","Pago","Estado","Acciones"].map((label)=><th key={label} style={{padding:"9px 10px",textAlign:label==="Valor"?"right":"left",color:"#756a5e",fontWeight:600,fontSize:11}}>{label}</th>)}
                        </tr>
                      </thead>
                      <tbody>
                        {historialPrestacionesEmpleado.length===0 ? (
                          <tr><td colSpan={7} style={{padding:18,textAlign:"center",color:"#a2988a"}}>Aún no hay prestaciones registradas para este empleado.</td></tr>
                        ) : historialPrestacionesEmpleado.map((prestacion)=>(
                          <tr key={prestacion.id} style={{borderBottom:"1px solid #e8dfd2"}}>
                            <td style={{padding:"9px 10px"}}>
                              <div style={{fontWeight:700,color:"#2b2622"}}>{PRESTACION_TIPOS_LABELS[prestacion.tipo]}</div>
                              <div style={{fontSize:10,color:"#756a5e",marginTop:3}}>{prestacion.observacion || "Registro contable por tercero"}</div>
                            </td>
                            ...
                            <td style={{padding:"9px 10px"}}>
                              <span style={{background:prestacion.estado==="pagada"||prestacion.estado==="consignada"?"#f4eee4":prestacion.estado==="en_nomina"?"#f4eee4":"#f4eee4",color:prestacion.estado==="pagada"||prestacion.estado==="consignada"?"#027a48":prestacion.estado==="en_nomina"?"#574e44":"#b54708",borderRadius:999,padding:"4px 10px",fontSize:10,fontWeight:700}}>
                            ...
                            <td style={{padding:"9px 10px"}}>
                              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                                ...
                                {prestacion.liquidacionEnNomina && (
                                  <span style={{fontSize:10,color:"#574e44",fontWeight:700,alignSelf:"center"}}>Se paga en nómina</span>
                                )}
                                <button onClick={()=>quitarPrestacionSocial(empleadoPrestacionActivo, prestacion.id)} style={{...B("#f9e9e4","#cc0000"),fontSize:10,padding:"6px 10px"}}>Eliminar</button>
                              </div>
                            </td>
                          </tr>
                        ))}
            ...
            {eVac&&(
              <>
                {renderNominaEmpleadoCard({
                  ...
                  metrics:[
                    {label:"Salario base", value:fmt(eVac.salario), color:"#2b2622"},
                    {label:"Valor día", value:fmt(Math.round((Number(eVac.salario)||0)/30)), color:"#0f766e"},
                    {label:"Corte activo", value:periodoNomina.label, color:"#2b2622"},
                    {label:"Días a pagar", value:diasLiquidar + " días", color:"#574e44"},
                    {label:"Valor vacaciones", value:fmt(valorLiquidar), color:"#027a48"},
                    {label:"Saldo restante", value:saldoVacaciones + " días", color:"#574e44"},
                  ],
                })}
                <div style={{height:14}}/>
                <div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:12,padding:"14px 16px"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
                    ...
                    <div style={{background:"#fff",borderRadius:8,padding:"10px 12px",border:"1px solid #bbf7d0",alignSelf:"end"}}>
                      <div style={{fontSize:10,color:"#756a5e"}}>Valor a pagar</div>
                      <div style={{fontSize:18,fontWeight:700,color:"#027a48"}}>{fmt(valorLiquidar)}</div>
                      <div style={{fontSize:10,color:"#756a5e",marginTop:4}}>Saldo restante: {saldoVacaciones} días</div>
                    </div>
                  </div>
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={()=>registrarVacacionesPagadas(eVac)} style={{...B("#2b2622","#2b2622"),fontSize:11,flex:1,justifyContent:"center"}}>Guardar cambios</button>
            ...
            {eCont&&pf&&resumenCorte&&(
              <>
                {renderNominaEmpleadoCard({
                  ...
                  badgeBg:eCont.activo?"#f4eee4":"#f9e9e4",
                  ...
                  metrics:[
                    {label:"Antigüedad", value:pf.diasTrabajados + " días", color:"#2b2622", hint:pf.mesesTrabajados + " meses · Ingreso " + (eCont.fechaIngreso||"No registrada")},
                    {label:"Días corte", value:resumenCorte.diasNomina + " días", color:"#574e44"},
                    {label:"Neto corte", value:fmt(resumenCorte.neto), color:"#0f766e"},
                    {label:"Descuentos", value:fmt(resumenCorte.totalDeducciones), color:"#574e44"},
                    {label:"Vacaciones pendientes", value:(vacacionesPendientesRetiro?.dias||0) + " días", color:"#027a48"},
                    {label:"Estado planilla", value:liquidacion?.retiroEnPeriodo?"Incluida":"Pendiente", color:liquidacion?.retiroEnPeriodo?"#027a48":"#756a5e"},
                  ],
                })}
                <div style={{height:14}}/>
                <div style={{background:"#fffbeb",border:"1px solid #fde68a",borderRadius:12,padding:"14px 16px"}}>
                  ...
                  <div style={{background:"#f0fdf4",borderRadius:6,padding:"8px 10px",marginBottom:10,display:"flex",alignItems:"center",gap:12,fontSize:11}}>
                    ...
                    <span style={{color:"#756a5e",fontSize:10}}>Disponibles: {vacacionesPendientesRetiro?.dias||0} días · Valor: {fmt(vacValorReal||0)}</span>
                  </div>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,marginBottom:10}}>
                    <thead><tr style={{background:"#2b2622",color:"#fff"}}><th style={{padding:"5px 8px",textAlign:"left"}}>Concepto</th><th style={{padding:"5px 8px",textAlign:"left"}}>Base cálculo</th><th style={{padding:"5px 8px",textAlign:"right"}}>Valor</th></tr></thead>
                    <tbody>
                      {[
                        ...
                        <tr key={k} style={{borderBottom:"1px solid #f4eee4"}}><td style={{padding:"5px 8px"}}>{k}</td><td style={{padding:"5px 8px",color:"#756a5e",fontSize:10}}>{b}</td><td style={{padding:"5px 8px",textAlign:"right",fontWeight:600,color:"#2b2622"}}>{fmt(v)}</td></tr>
                      ))}
                    </tbody>
                  ...
                  <div style={{display:"flex",gap:6,marginBottom:6}}>
                    ...
                    <button onClick={()=>printLiquidacion(eCont,pf,indemn||0,dvp,eCont.fechaSalida||null,resumenLiquidacion,periodoLiquidacion)} style={{...B("#2b2622","#b54708"),fontSize:11,flex:1,justifyContent:"center"}}>Imprimir liquidación</button>
      ...
      {tab==="colillas"&&(
        <div>
          <div style={{display:"flex",gap:12,alignItems:"center",justifyContent:"space-between",marginBottom:16,background:"#f4eee4",border:"1px solid #fed7aa",borderRadius:12,padding:"12px 14px"}}>
            <div>
              <div style={{fontSize:11,fontWeight:700,color:"#574e44",textTransform:"uppercase",letterSpacing:0.7}}>Colillas de pago</div>
            ...
            <button style={B("#2b2622","#2b2622")} onClick={()=>resumenesActivos.forEach(({empleado:e,resumen})=>printColilla(e,resumen,periodoNomina))}>🧾 Imprimir colillas masivas</button>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            {resumenesActivos.map(({empleado:e,resumen},i)=>{
              return(
              <div key={e.id} style={{background:"#fbf8f3",borderRadius:10,padding:"14px 16px",border:"1px solid #e8dfd2"}}>
                <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:10}}>
                  <Av init={e.avatar} color={PAL[i%PAL.length]} size={32}/>
                  <div style={{flex:1}}>
                    <div style={{fontSize:13,fontWeight:700}}>{e.nombre}</div>
                    <div style={{fontSize:10,color:"#756a5e"}}>{e.cargo} · {periodoNomina.label}</div>
                  </div>
                  <div style={{textAlign:"right",fontSize:12,fontWeight:700,color:"#2b2622"}}>{fmt(resumen.neto)}</div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,fontSize:10,marginBottom:12}}>
                  {[["Días laborados",resumen.diasNomina,"#574e44"],["Días incapacidad",resumen.diasIncapacidad,"#cc0000"],["Salario",fmt(resumen.salario),"#2b2622"],["Incapacidades",fmt(resumen.incapacidadTotal),"#027a48"],["Aux. transp.",fmt(resumen.auxilioTransporte),"#60b4ff"],["Extras + com.",fmt(resumen.horasExtras+resumen.comisiones),"#f59e0b"],["Base salud/pens.",fmt(resumen.baseSaludPension),"#574e44"],["Deducciones",fmt(resumen.totalDeducciones),"#cc0000"]].map(([k,v,col])=>(
                    <div key={k} style={{background:"#fff",borderRadius:8,padding:"8px 9px",border:"1px solid #e8dfd2"}}>
                      <div style={{color:"#a2988a",fontSize:9,textTransform:"uppercase",letterSpacing:0.5}}>{k}</div>
                      <div style={{fontWeight:700,color:col,fontSize:11,marginTop:3}}>{v}</div>
                    </div>
                  ))}
                </div>
                <div style={{display:"flex",gap:6}}>
                  <button onClick={()=>printColilla(e,resumen,periodoNomina)} style={{...B("#2b2622","#b54708"),fontSize:11,flex:1,justifyContent:"center"}}>🖨 Ver / imprimir colilla</button>
                  {e.tel&&<button onClick={()=>window.open("https://wa.me/57" + (Array.from(e.tel).filter(ch=>ch>="0"&&ch<="9").join("")) + "?text=" + (encodeURIComponent("Hola "+e.nombre+", adjuntamos tu colilla de pago del corte "+periodoNomina.label+". Neto a pagar: "+fmt(resumen.neto)+". Att. Ingeanclajes S.A.S")),'_blank')} style={{...B("#027a48","#2b2622"),fontSize:11,justifyContent:"center"}}>WhatsApp</button>}
                </div>
              </div>
              );
            })}
                    ...
                    {empleadoHEActivo && resumenHE ? (
                      <>
                        {renderNominaEmpleadoCard({
                          ...
                          metrics:[
                            {label:"Salario base", value:fmt(empleadoHEActivo.salario), color:"#2b2622"},
                            ...
                            {label:"Comisiones", value:fmt(resumenHE.comisiones), color:"#574e44"},
                            {label:"Base salud / pensión", value:fmt(resumenHE.baseSaludPension), color:"#574e44"},
                            {label:"Neto del corte", value:fmt(resumenHE.neto), color:"#0f766e"},
                          ],
                        })}

                        <div style={{background:"#ffffff",borderRadius:12,padding:"14px 16px",border:"1px solid #e8dfd2"}}>
                          <div style={{fontSize:11,fontWeight:700,color:"#2b2622",textTransform:"uppercase",marginBottom:10}}>Registrar movimiento</div>
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
                            <div><LBL>Obra</LBL>
                            ...
                            <div>
                              ...
                              <div style={{display:"flex",gap:8}}>
                                {[["horaExtra","Hora extra"],["comision","Comisión"]].map(([value,label])=>(
                                  <button key={value} onClick={()=>setHeForm({...heForm,tipo:value})} style={{...B(heForm.tipo===value?"#2b2622":"#2b2622",heForm.tipo===value?"#fff":"#a2988a"),flex:1,justifyContent:"center",border:"1px solid " + (heForm.tipo===value?"#2b2622":"#e8dfd2")}}>{label}</button>
                                ))}
                              </div>
                            </div>
                            {heForm.tipo==="horaExtra"&&(
                              <>
                                <div style={{gridColumn:"span 2"}}>
                                  ...
                                  {pctRecargo!==null&&<div style={{marginTop:4,fontSize:11,color:"#2b2622",fontWeight:600}}>Tarifa aplicada: {pctRecargo}% sobre hora ordinaria · Valor/hora calculado: {fmt(valorHoraRecargo)} (hora base: {fmt(valorHoraBase)})</div>}
                                </div>
                                ...
                                {pctRecargo===null
                                  ...
                                  : <div><LBL>Valor/hora (automático)</LBL><input type="number" value={valorHoraRecargo} readOnly style={{...SI,background:"#f4eee4",color:"#756a5e",cursor:"not-allowed"}}/></div>
                                }
                                <div style={{gridColumn:"span 2",background:"#fbf8f3",borderRadius:8,padding:"10px 12px",fontSize:12}}>Total {infoRec?.label||"horas extras"}: <strong style={{color:"#f59e0b"}}>{fmt(totalHE)}</strong></div>
                              </>
                            )}
                          ...
                          <div style={{display:"flex",justifyContent:"flex-end"}}>
                            <button style={B("#2b2622")} onClick={agregarHE}>Registrar</button>
                          </div>
                        </div>

                        <div style={{background:"#ffffff",borderRadius:12,padding:"14px 16px",border:"1px solid #e8dfd2"}}>
                          <div style={{fontSize:11,fontWeight:700,color:"#2b2622",textTransform:"uppercase",marginBottom:10}}>Movimientos del corte</div>
                          {horasPeriodo.length===0 && comisionesPeriodo.length===0 ? (
                            <div style={{fontSize:12,color:"#a2988a"}}>Este empleado no tiene horas extras ni comisiones registradas en el corte activo.</div>
                          ) : (
                            <div style={{display:"grid",gap:8}}>
                              {horasPeriodo.map((horaExtra)=>{
                                ...
                                return(
                                  <div key={horaExtra.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}>
                                    <div>
                                      <div style={{fontWeight:700,color:"#2b2622"}}>{recargo?.label||"Hora extra"}</div>
                                      <div style={{fontSize:11,color:"#756a5e"}}>{horaExtra.horas} h · {obra?.cliente||horaExtra.obraId} · {horaExtra.fecha}</div>
                                    </div>
                              ...
                              {comisionesPeriodo.map((comision)=>{
                                ...
                                return(
                                  <div key={comision.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}>
                                    <div>
                                      <div style={{fontWeight:700,color:"#2b2622"}}>{comision.concepto||"Comisión"}</div>
                                      <div style={{fontSize:11,color:"#756a5e"}}>{obra?.cliente||comision.obraId} · {comision.fecha}</div>
                                    </div>
                                    <div style={{fontWeight:700,color:"#574e44"}}>{fmt(comision.comision)}</div>
                                  </div>
                                );
                              })}
                            </div>
                      ...
                      <div style={{textAlign:"center",color:"#a2988a",padding:"28px 0"}}>
                    ...
                    {empleadoActivo && resumenNominaIncapacidad ? (
                      <>
                        {renderNominaEmpleadoCard({
                          ...
                          metrics:[
                            {label:"Días laborados", value:resumenNominaIncapacidad.diasNomina, color:"#574e44"},
                            {label:"Días incapacidad", value:resumenNominaIncapacidad.diasIncapacidad, color:"#2b2622"},
                            {label:"Empleador", value:fmt(resumenNominaIncapacidad.incapacidadEmpleador), color:"#574e44"},
                            {label:"EPS", value:fmt(resumenNominaIncapacidad.incapacidadEPS + resumenNominaIncapacidad.incapacidadEPS540), color:"#0f766e"},
                            {label:"Colpensiones", value:fmt(resumenNominaIncapacidad.incapacidadColpensiones), color:"#574e44"},
                            {label:"ARL", value:fmt(resumenNominaIncapacidad.incapacidadARL), color:"#f59e0b"},
                            {label:"Base salud / pensión", value:fmt(resumenNominaIncapacidad.baseSaludPension), color:"#574e44"},
                            {label:"Neto del corte", value:fmt(resumenNominaIncapacidad.neto), color:"#0f766e"},
                          ],
                        })}

                        <div style={{background:"#ffffff",borderRadius:12,padding:"14px 16px",border:"1px solid #e8dfd2"}}>
                          <div style={{fontSize:11,fontWeight:700,color:"#2b2622",textTransform:"uppercase",marginBottom:10}}>Registrar y calcular incapacidad</div>
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
                            <div>
                              <LBL>Origen</LBL>
                              <div style={{display:"flex",gap:8}}>
                                {[["comun","Origen común"],["laboral","Origen laboral"]].map(([value,label])=>(
                                  <button
                                    ...
                                    style={{...B(incapacidadForm.origen===value?"#2b2622":"#2b2622",incapacidadForm.origen===value?"#fff":"#a2988a"),flex:1,justifyContent:"center",border:"1px solid " + (incapacidadForm.origen===value?"#2b2622":"#e8dfd2")}}
                                  >
                                    {label}
                                  </button>
                                ))}
                            ...
                            <div>
                              ...
                              <input
                                ...
                                style={SI}
                              />
                              <div style={{fontSize:10,color:"#756a5e",marginTop:4}}>Úsalo cuando la incapacidad sea prórroga y ya vengan días reconocidos de certificados anteriores.</div>
                            </div>
                            <div>
                          ...
                          <div style={{background:"#fbf8f3",border:"1px solid #e8dfd2",borderRadius:10,padding:"12px 14px",fontSize:12,color:"#574e44",lineHeight:1.7,marginBottom:12}}>
                            <div style={{fontWeight:700,color:"#2b2622",marginBottom:6}}>Base legal aplicada para empresa privada</div>
                          ...
                          <div style={{display:"flex",justifyContent:"flex-end",gap:8,flexWrap:"wrap"}}>
                            <button type="button" onClick={calcularPreviewIncapacidad} style={B("#2b2622")}>Calcular incapacidad</button>
                        ...
                        {incapacidadPreview && previewRegistro ? (
                          <div style={{background:"#ffffff",borderRadius:12,padding:"14px 16px",border:"1px solid #e8dfd2"}}>
                            <div style={{fontSize:11,fontWeight:700,color:"#2b2622",textTransform:"uppercase",marginBottom:10}}>Vista previa del cálculo</div>
                            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:12}}>
                              <div style={{background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Origen</div><div style={{fontWeight:700,color:"#2b2622"}}>{INCAPACIDAD_ORIGEN_LABELS[previewRegistro.origen]}</div></div>
                              <div style={{background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Días en el corte</div><div style={{fontWeight:700,color:"#2b2622"}}>{previewRegistro.diasPeriodo}</div></div>
                              <div style={{background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Valor reconocido</div><div style={{fontWeight:700,color:"#027a48"}}>{fmt(previewRegistro.totalPeriodo)}</div></div>
                              <div style={{background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Neto estimado</div><div style={{fontWeight:700,color:"#0f766e"}}>{fmt(incapacidadPreview.nominaConPreview.neto)}</div></div>
                            </div>
                            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                              {Object.entries(previewRegistro.totalesResponsable)
                                .filter(([,valor])=>Number(valor||0)>0)
                                .map(([responsable, valor])=>(
                                  <div key={responsable} style={{background:"#f4eee4",border:"1px solid #fed7aa",borderRadius:8,padding:"10px 12px"}}>
                                    <div style={{fontSize:10,color:"#574e44"}}>{INCAPACIDAD_RESPONSABLE_LABELS[responsable]}</div>
                                    <div style={{fontWeight:700,color:"#574e44",marginTop:3}}>{fmt(valor)}</div>
                                  </div>
                                ))}
                            </div>
                          </div>
                        ...
                        <div style={{background:"#ffffff",borderRadius:12,padding:"14px 16px",border:"1px solid #e8dfd2"}}>
                          <div style={{fontSize:11,fontWeight:700,color:"#2b2622",textTransform:"uppercase",marginBottom:10}}>Incapacidades registradas en el corte</div>
                          {resumenIncapacidadActivo?.registros?.length ? (
                            <div style={{display:"grid",gap:8}}>
                              {resumenIncapacidadActivo.registros.map((registro)=>(
                                <div key={registro.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#fbf8f3",borderRadius:8,padding:"10px 12px",gap:12}}>
                                  <div style={{flex:1}}>
                                    <div style={{fontWeight:700,color:"#2b2622"}}>{INCAPACIDAD_ORIGEN_LABELS[registro.origen]} · {registro.fechaInicio}{registro.fechaFin!==registro.fechaInicio ? " al " + registro.fechaFin : ""}</div>
                                    <div style={{fontSize:11,color:"#756a5e"}}>{registro.diasPeriodo} día(s) en el corte · {registro.numeroSoporte || "Sin soporte"}{registro.diagnostico ? " · " + registro.diagnostico : ""}</div>
                                    <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:6,fontSize:10,color:"#574e44"}}>
                                      {Object.entries(registro.totalesResponsable)
                                        .filter(([,valor])=>Number(valor||0)>0)
                                        .map(([responsable, valor])=>(
                                          <span key={responsable} style={{background:"#fff",border:"1px solid #e8dfd2",borderRadius:999,padding:"4px 8px"}}>
                                  ...
                                  <div style={{textAlign:"right"}}>
                                    ...
                                    <button type="button" onClick={()=>quitarIncapacidad(empleadoActivo.id, registro.id)} style={{...B("#f9e9e4","#cc0000"),border:"1px solid #fecaca",padding:"6px 10px",fontSize:11,marginTop:8}}>Quitar</button>
                                  </div>
                                </div>
                              ))}
                            </div>
                          ) : (
                            <div style={{fontSize:12,color:"#a2988a"}}>Este empleado no tiene incapacidades aplicadas en el corte activo.</div>
                          )}
                        </div>
                      </>
                    ) : (
                ...
                <>
                  <div style={{background:"#fbf8f3",border:"1px solid #e8dfd2",borderRadius:12,padding:"14px 16px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{fontWeight:700,color:"#2b2622"}}>{empleadoDeduccionActivo.nombre}</div>
                        <div style={{fontSize:11,color:"#756a5e"}}>{empleadoDeduccionActivo.cargo || "Sin cargo"} · {empleadoDeduccionActivo.cedula || "Sin documento"} · {periodoNomina.label}</div>
                      </div>
                    </div>
                    {(()=>{
                      ...
                      return(
                        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
                          <div style={{background:"#fff",borderRadius:8,padding:"10px 12px",gridColumn:"span 3"}}>
                            <div style={{fontSize:10,color:"#756a5e"}}>Base salud / pensión del corte</div>
                            <div style={{fontWeight:700,color:"#2b2622",fontSize:16}}>{fmt(resumenDeduccion.baseSaludPension)}</div>
                            <div style={{fontSize:10,color:"#a2988a",marginTop:3}}>Incluye salario del corte + incapacidades + horas extras + comisiones.</div>
                          </div>
                          <div style={{background:"#fff",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Salud</div><div style={{fontWeight:700,color:"#2b2622"}}>{fmt(resumenDeduccion.salud)}</div></div>
                          <div style={{background:"#fff",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Pensión</div><div style={{fontWeight:700,color:"#e11d48"}}>{fmt(resumenDeduccion.pension)}</div></div>
                          <div style={{background:"#fff",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Incapacidades</div><div style={{fontWeight:700,color:"#027a48"}}>{fmt(resumenDeduccion.incapacidadTotal)}</div></div>
                          <div style={{background:"#fff",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Otras deducciones</div><div style={{fontWeight:700,color:"#574e44"}}>{fmt(resumenDeduccion.otrasDeducciones)}</div></div>
                          <div style={{background:"#fff",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Total descuentos</div><div style={{fontWeight:700,color:"#574e44"}}>{fmt(resumenDeduccion.totalDeducciones)}</div></div>
                          <div style={{background:"#fff",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Neto del corte</div><div style={{fontWeight:700,color:"#0f766e"}}>{fmt(resumenDeduccion.neto)}</div></div>
                          <div style={{background:"#fff",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:"#756a5e"}}>Extras + comisiones</div><div style={{fontWeight:700,color:"#f59e0b"}}>{fmt(resumenDeduccion.horasExtras + resumenDeduccion.comisiones)}</div></div>
                        </div>
                      );
                    })()}
                  </div>
                  ...
                  <div style={{background:"#ffffff",borderRadius:12,padding:"14px 16px",border:"1px solid #e8dfd2"}}>
                    <div style={{fontSize:11,fontWeight:700,color:"#2b2622",textTransform:"uppercase",marginBottom:10}}>Deducciones personalizadas</div>
                    {(empleadoDeduccionActivo.deduccionesPersonalizadas||[]).length>0 ? (
                      <div style={{display:"grid",gap:8,marginBottom:12}}>
                        {(empleadoDeduccionActivo.deduccionesPersonalizadas||[]).map((deduccion)=>(
                          <div key={deduccion.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}>
                            <div>
                              <div style={{fontWeight:700,color:"#2b2622"}}>{deduccion.nombre}</div>
                              <div style={{fontSize:11,color:"#756a5e"}}>{fmt(deduccion.valor)} mensual</div>
                            </div>
                            <button type="button" onClick={()=>quitarDeduccion(empleadoDeduccionActivo.id,deduccion.id)} style={{...B("#f9e9e4","#cc0000"),border:"1px solid #fecaca",padding:"6px 10px",fontSize:11}}>Quitar</button>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div style={{fontSize:11,color:"#756a5e",marginBottom:12}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1.2fr 1fr auto",gap:8,marginBottom:12}}>
                      ...
                      <button
                        ...
                        style={{...B("#2b2622"),justifyContent:"center",alignSelf:"end"}}
                      >
                        Agregar
                      </button>
                    </div>
                    <div style={{display:"flex",justifyContent:"flex-end"}}>
                      <button type="button" onClick={()=>guardarCambiosNomina("Cambios de deducciones sincronizados")} style={{...B("#2b2622","#2b2622")}}>Guardar cambios</button>
                    </div>
                    {mensajeGuardadoNomina && <div style={{fontSize:11,color:"#027a48",fontWeight:700,marginTop:8,textAlign:"right"}}>{mensajeGuardadoNomina}</div>}
                  </div>
                </>
        ...
        <div>
          <div style={{display:"flex",gap:12,alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
            <div style={{background:"#fbf8f3",border:"1px solid #e8dfd2",borderRadius:12,padding:"12px 14px"}}>
              <div style={{fontSize:11,fontWeight:700,color:"#574e44",textTransform:"uppercase",letterSpacing:0.7}}>Generación de nómina y plano banco</div>
              <div style={{fontSize:13,color:"#2b2622",marginTop:4}}>Corte activo: {periodoNomina.label}</div>
              <div style={{fontSize:11,color:"#756a5e",marginTop:4}}>
                {nominaEstaGenerada
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3, minmax(120px, 1fr))",gap:8,marginTop:12}}>
                <div style={{background:"#fff",borderRadius:10,padding:"9px 10px",border:"1px solid #f4eee4"}}>
                  <div style={{fontSize:9,color:"#756a5e",textTransform:"uppercase"}}>Registros nómina</div>
                  <div style={{fontWeight:700,color:"#2b2622",marginTop:4}}>{nominaVistaActual.totals.totalRegistros}</div>
                </div>
                <div style={{background:"#fff",borderRadius:10,padding:"9px 10px",border:"1px solid #f4eee4"}}>
                  <div style={{fontSize:9,color:"#756a5e",textTransform:"uppercase"}}>Listos para banco</div>
                  <div style={{fontWeight:700,color:"#027a48",marginTop:4}}>{nominaVistaActual.totals.totalRegistrosBanco}</div>
                </div>
                <div style={{background:"#fff",borderRadius:10,padding:"9px 10px",border:"1px solid #f4eee4"}}>
                  <div style={{fontSize:9,color:"#756a5e",textTransform:"uppercase"}}>Total banco</div>
                  <div style={{fontWeight:700,color:"#2b2622",marginTop:4}}>{fmt(nominaVistaActual.totals.totalBanco)}</div>
                </div>
              </div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:10,minWidth:230}}>
              ...
              <button
                style={{...B("#2b2622"),justifyContent:"center",padding:"13px 20px",fontSize:13.5,fontWeight:700}}
                onClick={generarNominaCorte}
              >
                {nominaEstaGenerada ? "Regenerar nómina" : "1 · Generar nómina"}
              </button>

              <div style={{fontSize:10.5,color:"#a2988a",lineHeight:1.45,textAlign:"center"}}>
                {nominaEstaGenerada
              ...
              <button
                style={{
                  ...B(nominaEstaGenerada?"#2b2622":"#f4eee4", nominaEstaGenerada?"#f4eee4":"#a2988a"),
              ...
              <button
                style={{...B("#fbf8f3","#574e44"),border:"1px solid #dbe4f0",justifyContent:"center",padding:"12px 20px",fontSize:12.5}}
          ...
          <div id="pz" className="doc-shell" style={{background:"#fff",color:"#111",fontFamily:"'Aptos','Segoe UI',sans-serif",fontSize:11,padding:"30px 40px",border:"1px solid #e8dfd2"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",borderBottom:"2px solid #2b2622",paddingBottom:14,marginBottom:20}}>
              <img src={LOGO_INGEANCLAJES} alt="Ingeanclajes" style={{height:60,objectFit:"contain"}}/>
              <div style={{textAlign:"right"}}><div style={{background:"#FFCD00",color:"#2b2622",padding:"6px 16px",borderRadius:4,fontWeight:700}}>BANCOLOMBIA</div><div style={{fontSize:10,color:"#756a5e",marginTop:4}}>Planilla Nómina · {nominaVistaActual.periodo.label}</div><div style={{fontSize:9,color:"#756a5e",marginTop:4}}>{nominaEstaGenerada ? ("Generada el " + formatNominaGeneratedAt(nominaVistaActual.generadoEn)) : "Vista previa del corte"}</div></div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12,marginBottom:16}}>
              <div style={{background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:9,color:"#756a5e"}}>Corte activo</div><div style={{fontWeight:700}}>{nominaVistaActual.periodo.label}</div></div>
              <div style={{background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:9,color:"#756a5e"}}>Días referencia</div><div style={{fontWeight:700}}>{nominaVistaActual.periodo.diasReferencia}</div></div>
              <div style={{background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:9,color:"#756a5e"}}>Nómina corte</div><div style={{fontWeight:700}}>{fmt(nominaVistaActual.totals.totalNomina)}</div></div>
              <div style={{background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:9,color:"#756a5e"}}>Liquidaciones corte</div><div style={{fontWeight:700,color:"#b54708"}}>{fmt(nominaVistaActual.totals.totalLiquidaciones)}</div></div>
              <div style={{background:"#fbf8f3",borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:9,color:"#756a5e"}}>Total a transferir</div><div style={{fontWeight:700,color:"#2b2622"}}>{fmt(nominaVistaActual.totals.totalPagar)}</div></div>
            </div>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{background:"#2b2622",color:"#fff"}}>{["#","Empleado","Documento","Banco / Cuenta","Días","Básico","Incap.","Aux. T.","H.Extra","Comisión","Deducciones","Liq. retiro","TOTAL"].map((h)=><th key={h} style={{padding:"6px 8px",textAlign:["Días","Básico","Incap.","Aux. T.","H.Extra","Comisión","Deducciones","Liq. retiro","TOTAL"].includes(h)?"right":"left",fontSize:10}}>{h}</th>)}</tr></thead>
              <tbody>
                {nominaVistaActual.registros.map(({empleado:e,resumen,liquidacionPrestaciones,totalPagar,retiroEnPeriodo,observacionesBanco},i)=>(
                  <tr key={e.id} style={{background:i%2===0?"#fff":"#f5f5f5",borderBottom:"1px solid #e0e0e0"}}>
                    <td style={{padding:"6px 8px",color:"#756a5e",fontSize:10}}>{i+1}</td>
                    <td style={{padding:"6px 8px",fontWeight:600,fontSize:10}}>
                      {e.nombre}
                      {retiroEnPeriodo && <div style={{fontSize:9,color:"#2b2622",fontWeight:700,marginTop:2}}>Liquidación incluida · retiro {e.fechaSalida}</div>}
                    </td>
                    <td style={{padding:"6px 8px",color:"#756a5e",fontSize:9}}>{e.cedula||"-"}</td>
                    <td style={{padding:"6px 8px",color:"#756a5e",fontSize:9}}>
                      {e.banco}<br/>{e.tipoCuenta}<br/><span style={{fontFamily:"monospace"}}>{e.numeroCuenta}</span>
                      {observacionesBanco?.length ? <div style={{fontSize:9,color:"#2b2622",marginTop:4}}>{observacionesBanco.join(" · ")}</div> : <div style={{fontSize:9,color:"#027a48",marginTop:4}}>Listo para banco</div>}
                    </td>
                    <td style={{padding:"6px 8px",textAlign:"right",fontSize:10}}>
                      <div>{resumen.diasNomina}</div>
                      {resumen.diasIncapacidad>0 ? <div style={{fontSize:9,color:"#2b2622"}}>{resumen.diasIncapacidad} inc.</div> : null}
                    </td>
                    ...
                    <td style={{padding:"6px 8px",textAlign:"right",color:"#2b2622",fontSize:9}}>
                    ...
                    <td style={{padding:"6px 8px",textAlign:"right",fontWeight:700,color:liquidacionPrestaciones>0?"#b54708":"#a2988a",fontSize:10}}>$ {Number(liquidacionPrestaciones || 0).toLocaleString("es-CO")}</td>
                    <td style={{padding:"6px 8px",textAlign:"right",fontWeight:700,color:"#2b2622",fontSize:10}}>$ {Number(totalPagar || 0).toLocaleString("es-CO")}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot><tr style={{background:"#2b2622",color:"#FFCD00"}}><td colSpan={12} style={{padding:"8px 10px",fontWeight:700,fontSize:11}}>TOTAL A PAGAR</td><td style={{padding:"8px 10px",textAlign:"right",fontWeight:700,fontSize:11}}>$ {Number(nominaVistaActual.totals.totalPagar || 0).toLocaleString("es-CO")}</td></tr></tfoot>
            </table>
            <div style={{marginTop:16,fontSize:10,color:"#756a5e",lineHeight:1.5}}>
            ...
            <div style={{marginTop:30,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:20}}>
              {["REPRESENTANTE LEGAL","CONTADOR","APROBADO POR"].map((l)=><div key={l} style={{textAlign:"center",borderTop:"1px solid #574e44",paddingTop:8}}><div style={{fontSize:10,color:"#756a5e"}}>{l}</div></div>)}
            </div>
          </div>
        </div>
      )}
