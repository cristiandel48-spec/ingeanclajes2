...
const barra = {
  ...
  background: "var(--surface-subtle, #f8fafc)", border: "1px solid var(--border, #e8edf4)", borderRadius: 11,
...
const control = {
  background: "var(--surface, #fff)", border: "1px solid var(--border, #dbe4f0)", borderRadius: 8,
  padding: "5px 8px", fontSize: 12, color: "var(--text-main, #142840)",
...
export default function PeriodoCorte({
  ...
  if (soloLectura) {
    return (
      <div style={{ ...barra, justifyContent: "flex-end" }}>
        ...
        <strong style={{ fontSize: 12.5, color: "var(--text-main, #142840)" }}>{periodo.label}</strong>
        <span style={{ fontSize: 11, color: "var(--text-muted, #94a3b8)" }}>· {periodo.diasReferencia} días</span>
      </div>
    );
  }
  ...
  return (
    <div style={barra}>
      ...
      <span style={{ fontSize: 11.5, color: "var(--text-muted, #64748b)", marginLeft: "auto" }}>
