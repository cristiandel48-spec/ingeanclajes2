...
export default function Vencimientos({ctx}){
  ...
  return(
    <div style={{padding:28}}>
      ...
      {/* Confirmación */}
      {guardado&&(
        <div style={{background:"#f4eee4",border:"1px solid #2b2622",borderRadius:10,padding:"12px 18px",marginBottom:20,fontSize:13,color:"#027a48",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:20,overflowY:"auto"}}>
          <div style={{background:"#fff",borderRadius:16,padding:32,width:"100%",maxWidth:580,boxShadow:"0 20px 60px rgba(0,0,0,0.35)",maxHeight:"90vh",overflowY:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
              <div>
                <div style={{fontSize:18,fontWeight:700,color:"#2b2622"}}>Generar Recertificación</div>
                <div style={{fontSize:12,color:"#756a5e",marginTop:4}}>Basada en: {recertForm.numero} · {recertForm.cliente}</div>
              </div>
              <button onClick={()=>setRecertForm(null)} style={{background:"#f4eee4",border:"none",borderRadius:8,padding:"6px 12px",cursor:"pointer",color:"#574e44",fontSize:13}}>Cerrar</button>
            </div>

            {/* Info del cert original */}
            <div style={{background:"#fbf8f3",borderRadius:10,padding:12,marginBottom:20,border:"1px solid #e8dfd2",fontSize:12}}>
              <div style={{color:"#756a5e",marginBottom:4}}>Certificado original</div>
              <div style={{fontWeight:700,color:"#2b2622"}}>{recertForm.cliente} · {recertForm.numero}</div>
              <div style={{color:"#574e44"}}>{recertForm.sistema}</div>
              <div style={{color:"#2b2622",marginTop:4}}>Vencía: {fmtD(recertForm.proxMant)||"sin fecha"}</div>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
              ...
              <div style={{gridColumn:"span 2",background:"#f4eee4",borderRadius:8,padding:12,border:"1px solid #e8dfd2"}}>
                <LBL>Próximo mantenimiento (obligatorio)</LBL>
                <input type="date" value={recertData.proxMant||""} onChange={e=>setRecertData(p=>({...p,proxMant:e.target.value}))} style={{...SI,border:"1px solid #e8dfd2"}}/>
                {recertData.proxMant&&<div style={{fontSize:11,color:"#2b2622",marginTop:6}}>Próximo mantenimiento: <strong>{fmtD(recertData.proxMant)}</strong></div>}
              </div>
            </div>

            {!recertData.proxMant&&<div style={{background:"#f9e9e4",borderRadius:6,padding:"8px 12px",fontSize:11,color:"#2b2622",marginBottom:12}}>Debes asignar la fecha del próximo mantenimiento para guardar.</div>}

            <div style={{display:"flex",gap:10}}>
              <button style={{...B("#2b2622"),flex:1,justifyContent:"center"}} onClick={guardarRecert}>
                Crear Recertificación
              </button>
              <button style={{...B("#f4eee4","#574e44"),flex:1,justifyContent:"center"}} onClick={()=>setRecertForm(null)}>
                Cancelar
              </button>
            </div>
          </div>
      ...
      {!certParaImpresion&&(
        <>
              ...
              <div key={g.titulo} style={{...CD,marginBottom:20,borderLeft:"4px solid " + (g.color)}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                  {items.map(c=>{
                    ...
                    return(
                      <div key={cActual.id}
                        style={{background:"#fbf8f3",borderRadius:10,padding:"14px 16px",border:"1px solid " + (colorV(dActual)) + "44",cursor:dActual!==null&&dActual<0?"pointer":"default",transition:"box-shadow 0.15s"}}
                        ...
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                          <div>
                            <div style={{fontSize:10,color:"#756a5e"}}>{cActual.id} · {cActual.numero}</div>
                            <div style={{fontSize:14,fontWeight:700,color:"#2b2622",marginTop:2}}>{cActual.cliente}</div>
                            <div style={{fontSize:11,color:"#574e44"}}>{cActual.tipo}</div>
                            {dActual!==null&&dActual<0&&(
                              <div style={{background:"#ef444422",border:"1px solid #e8dfd2",borderRadius:6,padding:"3px 8px",marginTop:6,fontSize:10,color:"#2b2622",fontWeight:700,display:"inline-block"}}>
                        ...
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
                          <div style={{background:"#fff",borderRadius:6,padding:"7px 10px"}}>
                            <div style={{fontSize:9,color:"#756a5e"}}>Certificado</div>
                            <div style={{fontSize:11,fontWeight:600}}>{fmtD(cActual.fecha)}</div>
                          </div>
                          <div style={{background:"#fff",borderRadius:6,padding:"7px 10px"}}>
                            <div style={{fontSize:9,color:"#756a5e"}}>Próx. mantenimiento</div>
                            <div style={{fontSize:11,fontWeight:600,color:colorV(dActual)}}>{fmtD(cActual.proxMant)||"—"}</div>
                          </div>
                        </div>
                        <div style={{fontSize:11,color:"#574e44",marginBottom:8,lineHeight:1.4}}>{cActual.sistema}</div>
                        {ob&&<div style={{fontSize:10,color:"#756a5e",marginBottom:10}}>{ob.direccion||ob.ciudad}</div>}
                        ...
                        <div style={{display:"flex",gap:6}}>
                          ...
                          <button
                            style={{...B("#f4eee4","#027a48"),border:"1px solid #2b2622",fontSize:11,padding:"7px 10px",flex:"0 0 auto",justifyContent:"center"}}
          ...
          {lista.filter(c=>c.diasRestantes===null).length>0&&(
            <div style={{...CD,borderLeft:"4px solid #756a5e"}}>
              <div style={{...ST,color:"#756a5e"}}>Sin fecha de mantenimiento asignada</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                {lista.filter(c=>c.diasRestantes===null).map(c=>(
                  <div key={c.id} style={{background:"#fbf8f3",borderRadius:10,padding:"14px 16px",border:"1px solid #e8dfd2"}}>
                    <div style={{fontSize:10,color:"#756a5e"}}>{c.id} · {c.numero}</div>
                    <div style={{fontSize:14,fontWeight:700}}>{c.cliente}</div>
                    <div style={{fontSize:11,color:"#574e44",marginBottom:12}}>{c.tipo} · {fmtD(c.fecha)}</div>
                    <div style={{display:"flex",gap:6}}>
                      <button style={{...B("#2b2622"),fontSize:11,flex:1,justifyContent:"center"}} onClick={()=>abrirRecert(c)}>
                        Generar recertificación
                      </button>
                      <button style={{...B("#f4eee4","#027a48"),border:"1px solid #2b2622",fontSize:11,flex:"0 0 auto",justifyContent:"center"}} onClick={()=>setImprimiendo(c)}>
                        Ver PDF
                      </button>
                    </div>
                  </div>
